...
async function runGeminiWebSearch(message) {
  if (!genAI) {
    throw new Error("GEMINI_API_KEY is not configured.");
  }
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (
      ...
      console.log(
        "🌐 Web Search → Google Search + Gemini"
      );

      const result =
        await runGeminiWebSearch(
          userMessage
        );
